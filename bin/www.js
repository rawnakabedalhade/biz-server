...
function onListening() {
  ...
  logger(chalk.green(`Listening on http://localhost:${bind}/`));
  connectToDb().then(async () => {
    //this func will be executed when i connected to db
    let bizId = await initialUsers();
    if (bizId) {
      await initialCards(bizId);
    }
  });
}
